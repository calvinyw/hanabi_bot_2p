...
const MAX_REBALANCE_LOOPS = 10;
const MAX_NORMAL_LOOPS = 10;
const HIGHEST_RANK = 4;
const PLAYABLE_WEIGHT = 8;
const TRASH_WEIGHT = 8;
const SAVED_WEIGHT = 3;
const REBALANCE_PASSES_PER_ITERATION = 3;
...
export function createClusterClueMapping(globalPossibleHands, gameState) {
  ...
  for (let iteration = 0; iteration < MAX_REBALANCE_LOOPS; iteration++) {
    ...
    clusterCounts = countAssignments(assignments);
    ...
    const rebalancedAssignments = rebalanceAssignments(
      ...
      centroidNorms,
      clusterCounts,
      REBALANCE_PASSES_PER_ITERATION,
    );
  ...
  for (let iteration = 0; iteration < MAX_NORMAL_LOOPS; iteration++) {
    const changedAssignments = assignHandsToClosestLegalClusters(
      possibleHandPoints,
      centroids,
      centroidNorms,
...
function updateCentroids(
  ...
  centroids.fill(0);
  const clusterCounts = countAssignments(assignments);
